...
(function () {
    $("#j-submit").on("click",function () {
        ...
        var txt = ue.getContent(),img = '', thumb=$("#thumb").val(),video = '', file="",type = $('#type').val(), type_name=$("#type").find("option:selected").text(),title =$("#title").val(),author=$("#author").val(),
            summary=$("#summary").val(),updateAt=new Date().toLocaleString().replace(/(\d{4}).(\d{1,2}).(\d{1,2})/mg, "$1-$2-$3").substr(0, 10);
        var _temp=$('#temp').html(txt).find("p");
            ...
            /*提交*/
            $.post(_url, {title: title, type: type, type_name: type_name,content: txt, img: img, video: video,file:file,author:author,thumb:thumb,summary:summary,updateAt:updateAt}, function (res) {
                alert(res.msg);
